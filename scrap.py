...
def get_info_pages(cat,day):
    #ON OUVRE LA PAGE
    f=io.open(dir_pages+get_name_page(cat,day),"r",encoding="utf-8")
    txt=f.read()
    f.close()
...
def an_page(cat):
    ...
    f=io.open(dir_pages+get_name_page(cat,ps[cats.index(cat)]),"r",encoding="utf-8")
    txt=f.read().lower()
    f.close()
...
def scrap_all_pages_of_cat(cat):
    ...
    day=1
    print("\ncat : ",cat)
    while etat!="404 Not Found" and sec<100:
        scrap_page(cat,day)
